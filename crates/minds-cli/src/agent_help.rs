...
fn card() -> serde_json::Value {
    serde_json::json!({
        ...
        "commands": [
            ...
            {"name": "fsck", "usage": "minds fsck [--require-review] [--require-seal]", "summary": "Ist jeder Trailer auflösbar? Journal-Lücken? Mit --require-review: Policy-Gate."},
            ...
            {"name": "reinterpret", "usage": "minds reinterpret <session>", "summary": "Gespeicherte Tool-Aufrufe mit dem aktuellen Adapter-Stand neu deuten — strikt lesend, Evidence unverändert."},
            {"name": "sign", "usage": "minds sign <session> [--key <pfad>] | minds sign --seal <seal-id> [--key <pfad>]", "summary": "Attribution einer Session signieren (nach stdout) oder einen Evidence-Seal nachsignieren (in den Store)."},
            {"name": "verify", "usage": "minds verify <session> [--signers <datei>] | minds verify <session> --sig <datei> | minds verify --evidence <seal-id>", "summary": "Evidence-Verdikt (Exit: 0 verifiziert, 1 manipuliert, 2 unvollständig, 3 nicht verifizierbar) oder signierte Attribution prüfen."},
            ...
            {"name": "audit", "usage": "minds audit --export [--out <datei>] [--base <ref>] [--mode redacted|proof]", "summary": "Provenienz-Kette als portables, ohne dieses Werkzeug prüfbares Bündel."},
